...
CREATE VIEW note_counts AS
  WITH groupnotes AS (
      SELECT
        ...
        ct.social_work
      FROM crosstab(
               'select  noteevents.hadm_id as hadm_id, noteevents.category as note_type, count(noteevents.text) as notes_count FROM `mimiciii_clinical.noteevents` where noteevents.hadm_id is not null GROUP BY noteevents.hadm_id,noteevents.category order by 1,2' :: TEXT,
               'select Distinct noteevents.category FROM `mimiciii_clinical.noteevents` order by 1' :: TEXT) ct(hadm_id INTEGER, case_management INTEGER, consult INTEGER, discharge_summary INTEGER, ecg INTEGER, echo INTEGER, general INTEGER, nursing INTEGER, nursing_other INTEGER, nutrition INTEGER, pharmacy INTEGER, physician INTEGER, radiology INTEGER, rehab_services INTEGER, respiratory INTEGER, social_work INTEGER)
  ), totalnotes AS (
      SELECT
        noteevents.hadm_id,
        count(noteevents.text) AS notes_count
      FROM `mimiciii_clinical.noteevents`
